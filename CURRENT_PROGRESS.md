...
- Phase 6 - Corpus Expansion And Reporting: in progress; report-summary coverage is implemented, the first compressed corpus expansion adds an RGB RLE Lossless Secondary Capture case, the next expansion adds a two-frame MONOCHROME2 RLE Lossless Secondary Capture case, the next expansion adds an odd-length RLE fragment padding Secondary Capture case, the next expansion adds modality-specific CT Image Storage and MR Image Storage RLE Lossless cases, the next expansion adds a Computed Radiography Image Storage RLE Lossless overlay/LUT case, the next expansion adds a Digital X-Ray Image Storage For Presentation RLE Lossless display-shutter case, and the latest expansion adds a Digital Mammography X-Ray Image Storage For Presentation RLE Lossless MONOCHROME1 12-bit case using the native project encoder without promoting deferred lossy or JPEG Extended 12-bit work.
...
- Phase 6 now includes a modality-specific Digital Mammography X-Ray Image Storage For Presentation RLE Lossless expansion case, `classic/mg/for_presentation_mono1_u16_12bit_rle_lossless`.
- The new case is default-build, byte-stable, generated through the native RLE encoder with one encapsulated fragment, preserves classic MG 12-bit unsigned MONOCHROME1 pixels, Presentation LUT Shape `INVERSE`, VOI window, detector/view/anatomy metadata, validates exact decoded native frame hashes, and appears in JSON reports with RLE codec metadata and compressed mammography stressors.
