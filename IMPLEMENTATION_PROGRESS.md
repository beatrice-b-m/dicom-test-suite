...
**Current implementation status:** Phase 0, Phase 0.5, Phase 1, Phase 2, Phase 3, and Phase 4 are functionally implemented, and Remediation R1 is complete. Remediation R2 has `list-cases --status` and `validate <generated-root>` implemented. Remaining remediation before clean Phase 5 expansion: `report` and `standards` CLI commands remain incomplete, validation needs stricter Part 10 checks, standards lock pinning is partial, and reproducibility/schema/CI guard coverage needs expansion
...
| Remediation before Phase 5 | in progress | R1 registry authority is complete and R2 list-case status filtering plus generated-root validation are implemented; remaining R2 CLI contracts and later remediation tasks in `REMEDIATION_PLAN.md` remain before adding new Phase 5 recipes. |
...
before Phase 5 feature expansion: `report` and `standards` CLI commands are not
implemented, validation does not yet enforce every required Part 10 invariant,
and reproducibility/schema/CI guard coverage is incomplete. The local
...
1. Continue Remediation R2 by implementing `report <generated-root> --format
   json` using `schemas/coverage-report.schema.json`.
