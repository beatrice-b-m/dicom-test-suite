...
| `IMPLEMENTATION_PLAN.md` | present | Phase 6 implementation plan with native transfer syntax foundation, deflated dataset feature-gating, encapsulated Pixel Data foundation, and retained Phase 5 history. |
...
| `Cargo.toml` / Rust workspace | present | Single package named `dicom-test-suite`, using Rust 2024 edition; pins minimal DICOM-rs crates and now exposes a project `deflate` feature wired to `dicom-object/deflate` and `dicom-transfer-syntax-registry/deflate`. |
...
- [x] Prepare concrete Phase 6 transfer syntax expansion plan in
      `IMPLEMENTATION_PLAN.md`.
- [x] Reverify DICOM-rs native transfer syntax dataset writer support for
      Implicit VR Little Endian, Explicit VR Little Endian, and Explicit VR Big
      Endian.
- [x] Update the transfer syntax capability matrix with DICOM-rs 0.9.1
      verification metadata and Explicit VR Big Endian availability.
...
- 2026-06-14: Phase 6 transfer syntax expansion started with planning and
  local capability verification. `IMPLEMENTATION_PLAN.md` now contains Phase
  6.0 native transfer syntax foundation, Phase 6.1 Deflated Explicit VR Little
  Endian feature-gating, and Phase 6.2 encapsulated Pixel Data foundation
  increments. `dicom-standard-kb` MCP lookups rechecked Explicit VR Big
  Endian, Deflated Explicit VR Little Endian, JPEG Baseline 8-bit, and RLE
  Lossless transfer syntax UIDs against PS3.6 Table A-1. Local DICOM-rs 0.9.1
  inspection verified built-in data-set read/write support for Implicit VR
  Little Endian, Explicit VR Little Endian, and retired Explicit VR Big
  Endian; optional compressed syntaxes remain feature-gated or adapter-gated.
  The capability matrix now records DICOM-rs 0.9.1 verification metadata and
  marks Explicit VR Big Endian as `available` for a future `legacy` recipe.
- 2026-06-14: Phase 6.0 generator transfer syntax abstraction is in place.
  `src/generator.rs` now uses `TransferSyntaxSpec` constants for the available
  native matrix-backed transfer syntaxes instead of scattering raw UID/name
  pairs through Part 10 write sites, validation expectations, and manifest
  DICOM metadata. The existing mammography recipe now selects a named transfer
  syntax spec for its Explicit VR Little Endian and Implicit VR Little Endian
  variants, and a unit test verifies that the generator specs match
  `transfer-syntax/capability-matrix.json` by keyword, UID, name, availability,
...
- 2026-06-14: Phase 6.1 Deflated Explicit VR Little Endian feature-gating is
  in place without flipping any generated deflated dataset case to
  `implemented`. `Cargo.toml` now exposes a project `deflate` feature wired to
  `dicom-object/deflate` and `dicom-transfer-syntax-registry/deflate`, with
  the resulting `flate2` dependency chain locked in `Cargo.lock`.
  `cases/registry.json` now includes
  `classic/sc/mono2_u8_deflated_explicit_le` as a planned `extended` Secondary
  Capture Image case using transfer syntax `1.2.840.10008.1.2.1.99`,
  determinism `semantic_stable`, and `requirements.features=["deflate"]`.
  Generation now records active project feature flags in the manifest and
  reports planned cases with feature requirements as
  `feature_gated_case_planned` unavailable rows; coverage reports count those
  rows as `planned` and include the deflated transfer syntax in grouped
...
- 2026-06-14 Phase 6.1 Deflated Explicit VR Little Endian feature-gate slice:
  - `dicom-standard-kb` MCP lookups rechecked Deflated Explicit VR Little
    Endian `1.2.840.10008.1.2.1.99` against PS3.6 Table A-1 and Secondary
    Capture Image Storage against PS3.4 Table B.5-1 before adding the planned
    registry row.
  - `cargo test deflated_transfer_syntax_is_feature_gated_in_cargo_and_registry`
    passed.
  - `cargo test report_counts_feature_gated_planned_cases_as_planned` passed.
  - `cargo test skipped_cases_use_registry_status_and_skip_metadata` passed.
  - `cargo fmt -- --check` passed after applying `cargo fmt` to the new Rust
    code.
  - `cargo test generate_command_writes_extended_enhanced_ct_multiframe_case`
    passed.
  - `cargo test generate_command_writes_all_profile_union_and_skips_planned_cases`
    passed.
  - `cargo test list_cases_command_filters_by_status_and_profile` passed.
  - `cargo test list_cases_filters_by_profile_and_status` passed.
  - `cargo test` passed with 89 tests.
  - `cargo test --features deflate` passed with 89 tests, confirming the new
    feature gate compiles and runs against the pinned DICOM-rs deflate feature
    set.
  - `cargo run -- standards check-lock` passed with the existing documented
    unavailable-pin warnings.
  - `cargo run -- list-cases --profile extended --status planned` passed and
    listed only `classic/sc/mono2_u8_deflated_explicit_le` as planned with
    `2/2` standards evidence covered.
  - `cargo run -- generate --profile extended --out /tmp/dts-phase6-deflate-slice --seed 1`
    passed, writing 17 generated files and recording the planned deflated case
    as feature-gated unavailable.
  - `cargo run -- validate /tmp/dts-phase6-deflate-slice` passed with 17 files
    checked and 0 validation failures.
  - `cargo run -- report /tmp/dts-phase6-deflate-slice --format json` passed
    with counts `generated=17`, `planned=1`, `skipped=0`, `blocked=0`; the
    planned row reports transfer syntax `1.2.840.10008.1.2.1.99` and
    validation status `unavailable`.
  - `cargo run -- report /tmp/dts-phase6-deflate-slice --format markdown`
    passed with the same counts and a gap row naming the missing active
    `deflate` feature and pending write/read/reproducibility verification.
  - `cargo run -- standards gaps --profile extended` passed and reported only
    the TSV header, confirming no standards evidence gaps.
...
- 2026-06-14 Phase 6.0 generator transfer syntax abstraction slice:
  - `dicom-standard-kb` MCP lookups rechecked Implicit VR Little Endian,
    Explicit VR Little Endian, and retired Explicit VR Big Endian transfer
    syntax UIDs against PS3.6 Table A-1.
  - `cargo test generator::tests::transfer_syntax_specs_are_backed_by_capability_matrix`
    passed with 1 focused test.
  - `cargo fmt -- --check` passed.
  - `cargo test` passed with 85 tests.
  - `cargo run -- standards check-lock` passed with the existing documented
    unavailable-pin warnings.
  - `cargo run -- generate --profile core --out /tmp/dts-transfer-syntax-slice --seed 1`
    passed, writing 19 files.
  - `cargo run -- validate /tmp/dts-transfer-syntax-slice` passed with 19
    files checked and 0 validation failures.
  - `cargo run -- report /tmp/dts-transfer-syntax-slice --format json` passed
    with counts `generated=19`, `planned=2`, `skipped=0`, `blocked=0`.

- 2026-06-14 Phase 6 transfer syntax capability planning slice:
  - `dicom-standard-kb` MCP lookups rechecked Explicit VR Big Endian,
    Deflated Explicit VR Little Endian, JPEG Baseline 8-bit, and RLE Lossless
    transfer syntax UIDs against PS3.6 Table A-1. Explicit VR Big Endian was
    reported as retired; the others were reported as non-retired transfer
    syntaxes.
  - Local DICOM-rs 0.9.1 crate inspection confirmed built-in native data-set
    encoders for Implicit VR Little Endian, Explicit VR Little Endian, and
    Explicit VR Big Endian; `cargo tree -e features` confirmed this crate does
    not currently enable optional compressed pixel codec features.
  - `cargo test --test project_artifacts` passed with 11 tests, including the
    new native transfer syntax matrix guard.
  - `cargo fmt -- --check` passed.
  - `cargo test` passed.
  - `cargo run -- standards check-lock` passed with the existing documented
    unavailable-pin warnings.
  - `git diff --check` passed.
  - Generated-output CLI checks were not run for this slice because no case
    registry row, generator recipe, validator path, report logic, or generated
    DICOM behavior changed.
