...
**Current implementation status:** Phase 0, Phase 0.5, Phase 1, Phase 2, Phase 3, and Phase 4 are functionally implemented, and Remediation R1 and R2 are complete. Remediation R3 has raw Part 10 byte-level validation, parsed cross-field image invariant validation, generated manifest schema-conformance checks, baseline standards-derived Type 1/Type 2 module validation, and initial SC/CT family-specific standards-derived validation implemented. Remediation R4 has core/extended byte-stability, `all` profile union/schema/skipped-case coverage, and generated-payload tracking/staging guard coverage implemented. Remaining remediation before clean Phase 5 expansion: validation needs broader family-specific standards-derived checks, standards lock pinning is partial, and registry consistency guard coverage needs expansion
...
| Remediation before Phase 5 | in progress | R1 registry authority and R2 required CLI contracts are complete; R3 raw Part 10 byte validation, parsed cross-field image invariant checks, manifest schema-conformance checks, baseline standards-derived Type 1/Type 2 checks, and initial SC/CT family-specific checks are implemented; R4 now covers core/extended byte-stability, `all` profile union/schema/skipped-case behavior, and generated-payload tracking/staging guards; additional validation hardening, registry consistency guard expansion, and standards lock pinning remain before adding new Phase 5 recipes. |
...
family-specific standards-derived checks, and registry consistency guard
...
2. Continue the remaining R4 registry consistency tests and standards-lock
   cleanup phases described in the remediation plan.
