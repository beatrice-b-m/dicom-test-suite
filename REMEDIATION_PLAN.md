...
- Add `list-cases --status <status>` and allow combining it with `--profile`.
- Implement `validate <generated-root>` as a first-class command that reads `manifest.json`, reopens each generated file, reruns internal validation, and reports failures with non-zero exit status.
- Implement `report <generated-root> --format json` using `schemas/coverage-report.schema.json`.
- Implement `report <generated-root> --format markdown` with the same coverage counts and gaps in human-readable form.
