...
| WSI independent reconstruction | `highdicom-wsi-reconstruction` | `python -m dts_wsi_reconstruction --input <file>` or three repeated `--group-input <file>` arguments | Required when collecting evidence for its declared WSI cases | The `uv`-locked highdicom 0.28.1 adapter independently hashes stored frames and reconstructs RGB matrices with every transform disabled. TILED_FULL uses independently derived implicit order; TILED_SPARSE validates explicit dimensions, positions, occupancy, payload, and zero-sentinel matrix. The locked group mode derives VOLUME, THUMBNAIL, and LABEL from attributes, binds Pyramid UID membership and shared identities, and proves the thumbnail reduction while excluding LABEL. |
...
`non-image/rt/radiation_set_minimal`. Locked `dciodvfy` and PixelMed identify
neither current IOD, so their unsupported result cannot serve as an IOD
...
`+F 1 -S -bs -M -W +Pid -O +op` policy must produce one 2 by 2 binary P6
