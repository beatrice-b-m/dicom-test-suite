...
| WSI independent reconstruction | `highdicom-wsi-reconstruction` | `python -m dts_wsi_reconstruction --input <file>` | Required when collecting evidence for its two exact WSI cases | The `uv`-locked highdicom 0.28.1 adapter independently hashes stored frames and reconstructs the 4 by 4 RGB matrix with every transform disabled. TILED_FULL uses independently derived implicit order; TILED_SPARSE validates the dimension organization and indices, explicit per-frame positions and ordinals, absent-tile occupancy, Pixel Data payload hash, and zero-sentinel matrix. |
