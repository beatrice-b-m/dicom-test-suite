...
| Case recipe | `0.1.0` and `0.2.0` | Version `0.2.0` adds explicit caller-owned Enhanced metadata/PET quantitation and native U1/U32 SC metadata/pixels, and typed PN/string/sequence metadata contracts. Readers select the declared schema; the frozen `0.1.0` schema and transitional resource identity remain unchanged. Current schema/resource identity includes both schemas. |
