...
| Case recipe | `0.1.0` and `0.2.0` | Version `0.2.0` adds explicit caller-owned Enhanced metadata/PET quantitation and native U1/U32 SC metadata/pixels, and typed VL/ICC and PN/string/sequence metadata contracts. Readers select the declared schema; the frozen `0.1.0` schema and transitional resource identity remain unchanged. Current schema/resource identity includes both schemas. |
...
The additional DX/MG tuple uses the same registry/provider restrictions,
`content.native_pixels`, `algorithm.classic_dx_mg`, DX/MG projection and one
matching `classic/dx/for-presentation`, `classic/mammography/for-presentation`
or `classic/mammography/for-processing` template at `1.0.0`. Strict typed
parameters and the single `instance`/order-zero contract remain required.
Caller case/recipe names, unique planning order and explicit path do not dispatch
the family. This is an additive accepted-input capability in the unreleased
0.2.0 line; existing recipe/template versions and historical payload hashes
remain unchanged. The dimension-overflow rejection is a compatible malformed-
input safety fix. Neither change qualifies a release or independent conformance.

The bounded metadata SC tuple adds caller-name independence for typed UTF-8
Person Name, qualified EmptyType2 and PrivateCreators under the documented
monochrome SC template@1, content, encoding and validation contract. Exact UTF-8
round trips and qualified Type2 tag/keyword/VR subsets reject malformed inputs;
checked high-bit arithmetic prevents overflow. This is an additive accepted-
input capability and compatible input-safety correction in unreleased0.2.0.
Recipe/template versions and historical payloads are unchanged. Existing
ISO2022, timezone, string-boundary and sequence-length admission remains scoped
as before. Public consumer proof does not update a corpus generator pin.

The bounded CR tuple uses native U8/OB pixels, one classic/cr@1 artifact and
explicit overlay/four-entry modality/VOI LUT parameters under the shared MR/CR
algorithm. Complete typed inspection takes precedence over historical names.
Its [generation contract](generation-guide.md#caller-defined-computed-radiography)
...
The bounded native MR tuple admits a nonempty multi-instance series through
the complete classic/mr@1 native-content and shared MR/CR algorithm/projection
contract. It binds caller metadata, shared MR acquisition values, unique
logical IDs/roles/paths and instance numbers, contiguous artifact order,
checked U16 pixels, and consistent orthonormal slice geometry. Its
[generation contract](generation-guide.md#caller-defined-native-mr-series)
does not interpret case/recipe names. The embedded oblique payload oracle and
...
[generation contract](generation-guide.md#caller-defined-native-pet) changes
neither recipe/template versions nor the accepted original payload bytes.
External bundle1 and manifest/report2 versions remain unchanged within
unreleased0.2.0. Pinned corpus binaries and historical evidence retain their
identities; source and caller payload identities are recorded separately.
Synthetic BQML/rescale evidence does not establish SUV or clinical accuracy.

The bounded native XA/XRF tuples similarly admit caller identities, orders and
safe paths while preserving each source 4×4 U8 projection, nine synthetic
provider fields and lexical geometry contract. The
[XA/XRF contract](generation-guide.md#caller-defined-native-xa-and-xrf) changes
no recipe/template or external bundle1/manifest2/report2 versions within
unreleased0.2.0. Original source payloads and pinned evidence retain their
identities; caller payload identities are separate. Native single-plane evidence
does not extend calibrated geometry, cine, RLE or enhanced XA/XRF qualification.

The two bounded photographic recipes also admit caller identities, unique
orders and safe paths under the complete
[photographic contract](generation-guide.md#caller-defined-native-photographic-images).
Their source-fixed 2×2 RGB/palette pixels, nine metadata fields and full artifact
projection remain unchanged. Original payload identities and pinned corpus
receipts remain separate from caller output identities. This changes no
recipe/template, bundle1, manifest2/report2 or product version and does not
widen the catalog's direct-composition RGB8 qualification or `rgb()` default.
Historical ICC/RLE routes retain exact source contracts; no arbitrary palette,
codec or independent conformance qualification is implied.
