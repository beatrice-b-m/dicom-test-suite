...
executables, provider responses, encoded frames, and bulk payloads are
untrusted. Template descriptors, committed schemas, the standards lock, and
...
- provider wall time, CPU time where available, resident memory, open files,
  process count, and output bytes; and
...
new private sibling staging directory with owner-only permissions, a random
...
Promotion is one same-filesystem atomic rename from the complete private
...
dependencies, codecs, validators, or providers. Local provider processes are
started with network access prohibited by the strongest available platform
sandbox. If the configured platform cannot enforce the provider's declared
`network = prohibited` requirement, provider execution is unavailable unless a
separately named qualification mode explicitly records the weaker boundary.
The ordinary supported composition workflow never implies network isolation
...
- passes a minimal fixed argument vector and no caller-controlled options
  outside the typed protocol;
- clears the inherited environment, then supplies only documented locale,
  temporary-directory, protocol, and staging variables;
- sets a private working directory and restrictive file-creation mask;
...
- enforces wall-clock timeout and available OS resource limits;
...
transaction with a stable reason code.
...
corpus in memory. Codec adapters that require a full frame may allocate at most
one envelope-checked frame per worker; unsupported frame sizes are reported as
`codec_frame_memory_limit`.
...
Every error is classified with a stable code and the failing logical instance,
slot, or provider where applicable. Diagnostics may include normalized relative
