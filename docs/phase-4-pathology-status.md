...
## WSI tile-referencing segmentation milestone

`derived/seg/wsi_tile_reference` completes milestone 6 as a semantic-stable
`extended` case generated by the locked highdicom/pydicom backend. Its one
FRACTIONAL OCCUPANCY Segmentation Storage instance references exactly Frames 1
and 4 of `vl/wsi/tiled_full_small`. Two 2 by 2 stored Frames occupy the diagonal
positions of a 4 by 4 `TILED_SPARSE` total pixel matrix; source Frames 2 and 3
are deliberately absent from the derivation graph. Strict validation binds the
source identities, Functional Groups, dimension ordering, tile positions,
fractional values, exact Frame and payload hashes, and the reconstructed matrix.

Two independent seed-7 extended generations each wrote 113 files and passed
strict validation with zero failures. Their semantic projections have identical
SHA-256
`97f63fbf891f85de6280807fe756acf5314b8005525f49c51995f09f57244f32`.
The 4,140-byte SEG is byte-identical between roots with SHA-256
`9d71aae5459e9d66984ee5706fc2bdcb35d7b3773ffaf93908217ae4bb117f41`.
Observed backend invocation times were 448 and 387 milliseconds, below the
locked five-second ceiling, and the instance remains below its 16 KiB ceiling.

Locked `dciodvfy` and the separately implemented authorized `uv`-locked
dicom-validator 0.8.2 adapter both report zero IOD errors, and DCMTK parses the
instance cleanly. The case-specific adapter version 0.2.0 has composite SHA-256
`b8ddaa1105b44fed04f838e53e65c02eae2d1213ddb3badb144be028fcdb73f6`.
It applies fail-closed corrections to the exact hash-locked 2026b definitions
and rejected removal of each of the five locked Functional Group macros. Rust
independently reconstructs the two stored Frames and exact total matrix; the
generating backend is not its own payload or reference authority.

Exact-slice conformance run
`973749c21773cd7e66aae6c8377600f4a7ca839c4d88ec0fe45b256e9684c9bf`
records silent entity validation, passing independent source-WSI pixel evidence,
zero accepted findings, and zero verification failures against an empty
findings set. Unrelated whole-corpus findings remain visible and unallowlisted.
The registry now contains 150 implemented and 32 planned logical cases.

Full-corpus conformance run
`d9252ca2fa2edaad6d2c445f5cb0076a0a2b7558355ca9635a228bd0d3ca037d`
covered all 113 instances. Verification retained zero accepted findings and
reported 220 pre-existing or unrelated failures; none were hidden or newly
allowlisted for this milestone.

All six dependency-ordered Phase 4 milestones are complete. Ordinary `all`
profile semantics remain unchanged: it includes the promoted `extended` source
WSI and derived SEG, but not the opt-in stress-only full-size pyramid. Further
autonomous expansion pauses at the plan's explicit decision checkpoint for
selecting the dimensions, resource budgets, and ordinary-CI scheduling of a
genuinely full-size WSI pyramid.
