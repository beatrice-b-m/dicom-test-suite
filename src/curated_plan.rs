...
use crate::recipes::classic_vl_projection::{inspect_xa_xrf_capability, plan_vl_projection_recipe};
use crate::recipes::{
...
> {
    ...
    if inspect_xa_xrf_capability(recipe)
        .map_err(|error| CuratedPlanError::ClassicPlan {
            recipe_id: recipe.recipe_id.clone(),
            message: error.to_string(),
        })?
...
mod classic_ct_capability_tests {
    ...
    fn dx_mg_capability_dispatch_ignores_misleading_case_names() {
        ...
        for (case_id, recipe_id) in [
            ("classic/pet/rescaled_activity_explicit_le", "caller_cr"),
            ("vl/photo/rgb_planar0_explicit_le", "caller_cr"),
            ("caller/radiograph", "mr_multislice_oblique"),
        ] {
            let mut input = recipe("classic/cr/overlay_modality_voi_explicit_le");
            input.binding.case_id = case_id.into();
            input.recipe_id = recipe_id.into();
            input.planning_order = Some(900);
            input.dicom.as_mut().unwrap().artifacts[0].output.path =
                Some("images/radiograph.dcm".into());
            let direct = plan_mr_cr_recipe(&input, &lock_hash(), 1).unwrap().unwrap();
            assert_eq!(classic_requests(&input, &lock_hash(), 1).unwrap().0, direct);
            input.dicom.as_mut().unwrap().artifacts[0].algorithm_provider_id =
