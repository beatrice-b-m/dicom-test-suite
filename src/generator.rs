...
mod tests {
    ...
    fn execute_plan_first_case(run: &PreparedGenerationRun, case_id: &str) -> GeneratedFile {
        let bundle = crate::prepare_curated_plan_for_selection(
            crate::curated_plan::CuratedScSelection::CaseIds(vec![case_id.to_string()]),
            run.seed,
        )
        ...
        let mut files = crate::execute_curated_sc_plan_output(Some(&bundle), &run.out_dir)
    ...
    fn negative_profile_is_deterministic_isolated_and_removes_private_sources() {
        ...
        assert_eq!(first.files.len(), crate::negative::NEGATIVE_CASE_IDS.len());
        assert_eq!(first.files.len(), second.files.len());
        assert!(!first_root.path().join("private-sources").exists());
        assert!(!second_root.path().join("private-sources").exists());
