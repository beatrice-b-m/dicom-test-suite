...
fn inventory_exactly_covers_implemented_valid_dicom_sop_classes() {
    ...
    for case in registry["cases"].as_array().expect("registry cases") {
        ...
        let uid = case["sop_class_uid"]
            .as_str()
            .expect("valid instance SOP Class UID");
        let identity = (
    ...
    let mappings = inventory["mappings"]
        .as_array()
        .expect("inventory mappings");
    let mut actual = BTreeMap::new();
    let allowed_tasks = BTreeSet::from([
        "P2.1", "P3.2", "P3.3", "P3.4", "P3.5", "P3.6", "P3.7", "P5.4", "P5.5", "P5.6", "P5.7",
        "P6.2", "P6.3", "P6.4", "P6.5", "P6.6",
    ]);
    for mapping in mappings {
        let uid = mapping["sop_class_uid"]
            .as_str()
            .expect("mapped SOP Class UID");
        let identity = (
            mapping["sop_class_name"]
                .as_str()
                .expect("mapped SOP Class name"),
            mapping["iod_name"].as_str().expect("mapped IOD name"),
        );
        assert!(
            actual.insert(uid, identity).is_none(),
            "duplicate mapping for {uid}"
        );
        assert!(
            matches!(
                mapping["coverage_kind"].as_str(),
                Some("template" | "bundle")
            ),
            "{uid} must declare template or bundle coverage"
        );
        assert!(
            mapping["template_family"]
                .as_str()
                .is_some_and(|value| !value.is_empty()),
            "{uid} must name a template family"
        );
        assert!(
            mapping["qualification_owner"]
                .as_str()
                .is_some_and(|value| !value.is_empty()),
        ...
        assert!(
            mapping["phase_task"]
                .as_str()
                .is_some_and(|task| allowed_tasks.contains(task)),
    ...
    assert_eq!(
        actual, expected,
        "template inventory must exactly match registry scope"
    );
}
