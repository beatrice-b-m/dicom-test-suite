...
pub(crate) fn assert_curated_manifest_schema_valid(manifest: &Value) {
    let errors = validation_errors(manifest);
    assert!(errors.is_empty(), "manifest schema errors: {errors:?}");
}

pub(crate) fn curated_manifest_schema_is_valid(manifest: &Value) -> bool {
    validation_errors(manifest).is_empty()
}

pub(crate) fn assert_curated_manifest_schema_rejected(manifest: &Value) {
    let errors = validation_errors(manifest);
    assert!(
        !errors.is_empty(),
