...
fn generate_command_writes_core_u16_native_pixel_case() {
    ...
    let vl_photo_file = file_entry_by_case_id(&manifest, "vl/photo/rgb_planar0_explicit_le");
    assert_eq!(
        vl_photo_file
            .pointer("/dicom/sop_class_uid")
            .and_then(Value::as_str),
    ...
    assert!(
        out_dir
            ...
            .is_file(),
        "core generation should materialize the promoted XRF monoplane fixture"
    );
    let obj = open_file(&dcm_path).expect("generated DICOM file should parse");
    assert_eq!(
...
fn generate_command_writes_extended_enhanced_ct_multiframe_case() {
    ...
    assert_eq!(spatial_registration["determinism"], "byte_stable");
    assert_eq!(
    ...
    let rle_mono1_file = file_entry_by_case_id(&manifest, "classic/sc/mono1_u8_rle_lossless");
    assert_eq!(
        rle_mono1_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
    ...
    assert!(
        rle_mono1_file
            .pointer("/image/planar_configuration")
            .is_some_and(Value::is_null),
        "single-sample MONOCHROME1 RLE should not include Planar Configuration"
    );
    assert_eq!(
        rle_mono1_file
            .pointer("/pixel_data/native_or_encapsulated")
    ...
    assert!(
        validation_result_names(rle_mono1_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 RLE manifest should record decoded native frame hash validation"
    );
    ...
    let rle_mono1_u16_file = file_entry_by_case_id(&manifest, "classic/sc/mono1_u16_rle_lossless");
    assert_eq!(
        rle_mono1_u16_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
    ...
    assert!(
        rle_mono1_u16_file
            .pointer("/image/planar_configuration")
            .is_some_and(Value::is_null),
        "single-sample MONOCHROME1 16-bit RLE should not include Planar Configuration"
    );
    assert!(
        validation_result_names(rle_mono1_u16_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "16-bit MONOCHROME1 RLE manifest should record decoded native frame hash validation"
    );
    let rle_odd_3x3_file =
    ...
    let rle_mono1_odd_3x3_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono1_u16_odd_3x3_rle_lossless");
    assert_eq!(
        rle_mono1_odd_3x3_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert!(
        validation_result_names(rle_mono1_odd_3x3_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 unsigned odd 3x3 RLE manifest should record decoded native frame hash validation"
    );
    ...
    let rle_mono1_signed_odd_3x3_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono1_i16_odd_3x3_rle_lossless");
    assert_eq!(
        rle_mono1_signed_odd_3x3_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert!(
        validation_result_names(rle_mono1_signed_odd_3x3_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 signed odd 3x3 RLE manifest should record decoded native frame hash validation"
    );
    let rle_rect_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono2_u16_rect_2x3_rle_lossless");
    assert_eq!(
        rle_rect_file.pointer("/image/rows").and_then(Value::as_u64),
        Some(2)
    );
    assert_eq!(
        rle_rect_file
    ...
    assert!(
        validation_result_names(rle_rect_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "rectangular 2x3 RLE manifest should record decoded native frame hash validation"
    );
    let rle_mono1_rect_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono1_u16_rect_2x3_rle_lossless");
    assert_eq!(
        rle_mono1_rect_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
    ...
    assert!(
        validation_result_names(rle_mono1_rect_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 rectangular 2x3 RLE manifest should record decoded native frame hash validation"
    );
    let rle_signed_rect_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono2_i16_rect_2x3_rle_lossless");
    assert_eq!(
        rle_signed_rect_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME2")
    );
    ...
    assert!(
        validation_result_names(rle_signed_rect_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "signed rectangular 2x3 RLE manifest should record decoded native frame hash validation"
    );
    let rle_mono1_signed_rect_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono1_i16_rect_2x3_rle_lossless");
    assert_eq!(
        rle_mono1_signed_rect_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert!(
        validation_result_names(rle_mono1_signed_rect_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 signed rectangular 2x3 RLE manifest should record decoded native frame hash validation"
    );
    let rle_tiny_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono2_u16_tiny_1x1_rle_lossless");
    assert_eq!(
        rle_tiny_file.pointer("/image/rows").and_then(Value::as_u64),
        Some(1)
    );
    assert_eq!(
        rle_tiny_file
    ...
    let rle_signed_tiny_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono2_i16_tiny_1x1_rle_lossless");
    assert_eq!(
        rle_signed_tiny_file
            .pointer("/image/pixel_representation")
            .and_then(Value::as_u64),
        Some(1)
    );
    ...
    assert!(
        validation_result_names(rle_signed_tiny_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "signed tiny 1x1 RLE manifest should record decoded native frame hash validation"
    );
    let rle_mono1_signed_tiny_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono1_i16_tiny_1x1_rle_lossless");
    assert_eq!(
        rle_mono1_signed_tiny_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert!(
        validation_result_names(rle_mono1_signed_tiny_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 signed tiny 1x1 RLE manifest should record decoded native frame hash validation"
    );
    ...
    assert_eq!(
        rle_u8_padding_file.pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(0))
    );
    assert_eq!(
        rle_u8_padding_file.pointer("/recipe/recipe_parameters/pixel_padding/range_limit"),
        Some(&Value::from(0))
    );
    assert!(
        validation_result_names(rle_u8_padding_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_value"),
        "8-bit Pixel Padding RLE manifest should record Pixel Padding Value validation"
    );
    assert!(
        validation_result_names(rle_u8_padding_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_range_limit"),
        "8-bit Pixel Padding RLE manifest should record Pixel Padding Range Limit validation"
    );
    assert!(
        validation_result_names(rle_u8_padding_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "8-bit Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_mono1_u8_padding_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono1_u8_padding_rle_lossless");
    assert_eq!(
        rle_mono1_u8_padding_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert_eq!(
        rle_mono1_u8_padding_file.pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(0))
    );
    assert_eq!(
        rle_mono1_u8_padding_file.pointer("/recipe/recipe_parameters/pixel_padding/range_limit"),
        Some(&Value::from(0))
    );
    assert!(
        validation_result_names(rle_mono1_u8_padding_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_value"),
        "MONOCHROME1 8-bit Pixel Padding RLE manifest should record Pixel Padding Value validation"
    );
    assert!(
        validation_result_names(rle_mono1_u8_padding_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_range_limit"),
        "MONOCHROME1 8-bit Pixel Padding RLE manifest should record Pixel Padding Range Limit validation"
    );
    assert!(
        validation_result_names(rle_mono1_u8_padding_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 8-bit Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_u8_padding_multiframe_file = file_entry_by_case_id(
        &manifest,
        "classic/sc/mono2_u8_padding_multiframe_rle_lossless",
    );
    assert_eq!(
        rle_u8_padding_multiframe_file
            .pointer("/image/frames")
            .and_then(Value::as_u64),
        Some(2)
    );
    ...
    assert_eq!(
        rle_u8_padding_multiframe_file.pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(0))
    );
    assert_eq!(
        rle_u8_padding_multiframe_file
            .pointer("/recipe/recipe_parameters/pixel_padding/range_limit"),
        Some(&Value::from(0))
    );
    assert_eq!(
        rle_u8_padding_multiframe_file
    ...
    assert!(
        validation_result_names(rle_u8_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_value"),
        "8-bit multi-frame Pixel Padding RLE manifest should record Pixel Padding Value validation"
    );
    assert!(
        validation_result_names(rle_u8_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_range_limit"),
        "8-bit multi-frame Pixel Padding RLE manifest should record Pixel Padding Range Limit validation"
    );
    assert!(
        validation_result_names(rle_u8_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"number_of_frames"),
        "8-bit multi-frame Pixel Padding RLE manifest should validate Number of Frames"
    );
    assert!(
        validation_result_names(rle_u8_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "8-bit multi-frame Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_mono1_u8_padding_multiframe_file = file_entry_by_case_id(
        &manifest,
        "classic/sc/mono1_u8_padding_multiframe_rle_lossless",
    );
    assert_eq!(
        rle_mono1_u8_padding_multiframe_file
            .pointer("/image/frames")
            .and_then(Value::as_u64),
        Some(2)
    );
    ...
    assert_eq!(
        rle_mono1_u8_padding_multiframe_file
            .pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(0))
    );
    assert_eq!(
        rle_mono1_u8_padding_multiframe_file
    ...
    assert!(
        validation_result_names(
            rle_mono1_u8_padding_multiframe_file.pointer("/validation/internal")
        )
        .contains(&"number_of_frames"),
        "MONOCHROME1 8-bit multi-frame Pixel Padding RLE manifest should validate Number of Frames"
    );
    assert!(
        validation_result_names(
            rle_mono1_u8_padding_multiframe_file.pointer("/validation/internal")
        ...
        .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 8-bit multi-frame Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_padding_multiframe_file = file_entry_by_case_id(
        &manifest,
        "classic/sc/mono2_u16_padding_multiframe_rle_lossless",
    );
    assert_eq!(
        rle_padding_multiframe_file
            .pointer("/image/frames")
            .and_then(Value::as_u64),
        Some(2)
    );
    ...
    assert_eq!(
        rle_padding_multiframe_file.pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(0))
    );
    assert_eq!(
        rle_padding_multiframe_file.pointer("/recipe/recipe_parameters/pixel_padding/range_limit"),
        Some(&Value::from(0))
    );
    assert_eq!(
        rle_padding_multiframe_file
    ...
    assert!(
        validation_result_names(rle_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_value"),
        "multi-frame Pixel Padding RLE manifest should record Pixel Padding Value validation"
    );
    assert!(
        validation_result_names(rle_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_range_limit"),
        "multi-frame Pixel Padding RLE manifest should record Pixel Padding Range Limit validation"
    );
    assert!(
        validation_result_names(rle_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"number_of_frames"),
        "multi-frame Pixel Padding RLE manifest should validate Number of Frames"
    );
    assert!(
        validation_result_names(rle_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "multi-frame Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_mono1_padding_multiframe_file = file_entry_by_case_id(
        &manifest,
        "classic/sc/mono1_u16_padding_multiframe_rle_lossless",
    );
    assert_eq!(
        rle_mono1_padding_multiframe_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert_eq!(
        rle_mono1_padding_multiframe_file.pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(0))
    );
    assert_eq!(
        rle_mono1_padding_multiframe_file
    ...
    assert!(
        validation_result_names(rle_mono1_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"number_of_frames"),
        "MONOCHROME1 unsigned multi-frame Pixel Padding RLE manifest should validate Number of Frames"
    );
    assert!(
        validation_result_names(rle_mono1_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 unsigned multi-frame Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_signed_padding_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono2_i16_padding_rle_lossless");
    assert_eq!(
        rle_signed_padding_file
            .pointer("/image/pixel_representation")
            .and_then(Value::as_u64),
        Some(1)
    );
    ...
    assert_eq!(
        rle_signed_padding_file.pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(-32768))
    );
    assert_eq!(
        rle_signed_padding_file.pointer("/recipe/recipe_parameters/pixel_padding/range_limit"),
        Some(&Value::from(-32768))
    );
    assert_eq!(
        rle_signed_padding_file
    ...
    assert!(
        validation_result_names(rle_signed_padding_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_value"),
        "signed Pixel Padding RLE manifest should record Pixel Padding Value validation"
    );
    assert!(
        validation_result_names(rle_signed_padding_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_range_limit"),
        "signed Pixel Padding RLE manifest should record Pixel Padding Range Limit validation"
    );
    assert!(
        validation_result_names(rle_signed_padding_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "signed Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_mono1_signed_padding_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono1_i16_padding_rle_lossless");
    assert_eq!(
        rle_mono1_signed_padding_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert_eq!(
        rle_mono1_signed_padding_file.pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(-32768))
    );
    assert_eq!(
        rle_mono1_signed_padding_file
    ...
    assert!(
        validation_result_names(rle_mono1_signed_padding_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_value"),
        "MONOCHROME1 signed Pixel Padding RLE manifest should record Pixel Padding Value validation"
    );
    assert!(
        validation_result_names(rle_mono1_signed_padding_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_range_limit"),
        "MONOCHROME1 signed Pixel Padding RLE manifest should record Pixel Padding Range Limit validation"
    );
    assert!(
        validation_result_names(rle_mono1_signed_padding_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 signed Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_mono1_signed_padding_multiframe_file = file_entry_by_case_id(
        &manifest,
        "classic/sc/mono1_i16_padding_multiframe_rle_lossless",
    );
    assert_eq!(
        rle_mono1_signed_padding_multiframe_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert!(
        validation_result_names(
            rle_mono1_signed_padding_multiframe_file.pointer("/validation/internal")
        )
        .contains(&"number_of_frames"),
        "MONOCHROME1 signed multi-frame Pixel Padding RLE manifest should validate Number of Frames"
    );
    assert!(
        validation_result_names(
            rle_mono1_signed_padding_multiframe_file.pointer("/validation/internal")
        ...
        .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 signed multi-frame Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    let rle_signed_padding_multiframe_file = file_entry_by_case_id(
        &manifest,
        "classic/sc/mono2_i16_padding_multiframe_rle_lossless",
    );
    assert_eq!(
        rle_signed_padding_multiframe_file
            .pointer("/image/pixel_representation")
            .and_then(Value::as_u64),
        Some(1)
    );
    ...
    assert_eq!(
        rle_signed_padding_multiframe_file.pointer("/recipe/recipe_parameters/pixel_padding/value"),
        Some(&Value::from(-32768))
    );
    assert_eq!(
        rle_signed_padding_multiframe_file
    ...
    assert!(
        validation_result_names(rle_signed_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_value"),
        "signed multi-frame Pixel Padding RLE manifest should record Pixel Padding Value validation"
    );
    assert!(
        validation_result_names(rle_signed_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"pixel_padding_range_limit"),
        "signed multi-frame Pixel Padding RLE manifest should record Pixel Padding Range Limit validation"
    );
    assert!(
        validation_result_names(rle_signed_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"number_of_frames"),
        "signed multi-frame Pixel Padding RLE manifest should validate Number of Frames"
    );
    assert!(
        validation_result_names(rle_signed_padding_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "signed multi-frame Pixel Padding RLE manifest should record decoded native frame hash validation"
    );
    ...
    let rle_mono1_i16_file = file_entry_by_case_id(&manifest, "classic/sc/mono1_i16_rle_lossless");
    assert_eq!(
        rle_mono1_i16_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
    ...
    assert!(
        rle_mono1_i16_file
            .pointer("/image/planar_configuration")
            .is_some_and(Value::is_null),
        "single-sample signed MONOCHROME1 RLE should not include Planar Configuration"
    );
    assert!(
        validation_result_names(rle_mono1_i16_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "signed 16-bit MONOCHROME1 RLE manifest should record decoded native frame hash validation"
    );
    ...
    let rle_rgb_multiframe_file =
        file_entry_by_case_id(&manifest, "classic/sc/rgb_planar0_multiframe_rle_lossless");
    assert_eq!(
        rle_rgb_multiframe_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("RGB")
    );
    ...
    assert!(
        validation_result_names(rle_rgb_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "RGB multi-frame RLE manifest should record decoded native frame hash validation"
    );
    let rle_rgb_planar1_file =
        file_entry_by_case_id(&manifest, "classic/sc/rgb_planar1_rle_lossless");
    assert_eq!(
        rle_rgb_planar1_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("RGB")
    );
    ...
    assert!(
        validation_result_names(rle_rgb_planar1_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "RGB planar-1 RLE manifest should record decoded native frame hash validation"
    );
    let rle_rgb_planar1_multiframe_file =
        file_entry_by_case_id(&manifest, "classic/sc/rgb_planar1_multiframe_rle_lossless");
    assert_eq!(
        rle_rgb_planar1_multiframe_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("RGB")
    );
    ...
    assert!(
        validation_result_names(rle_rgb_planar1_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "RGB planar-1 multi-frame RLE manifest should record decoded native frame hash validation"
    );
    let rle_ybr_file = file_entry_by_case_id(&manifest, "classic/sc/ybr_full_planar0_rle_lossless");
    assert_eq!(
        rle_ybr_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
    ...
    assert!(
        rle_ybr_file
            ...
            .and_then(Value::as_array)
            .expect("YBR_FULL RLE manifest should include known stressors")
            .iter()
            .any(|stressor| stressor.as_str() == Some("ybr_full_pixels")),
        "YBR_FULL RLE case should label YBR_FULL pixels as a stressor"
    );
    assert!(
    ...
    let rle_ybr_multiframe_file = file_entry_by_case_id(
        &manifest,
        "classic/sc/ybr_full_planar0_multiframe_rle_lossless",
    );
    assert_eq!(
        rle_ybr_multiframe_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("YBR_FULL")
    );
    ...
    assert!(
        rle_ybr_multiframe_file
            .pointer("/known_stressors")
            .and_then(Value::as_array)
            .expect("YBR_FULL multi-frame RLE manifest should include known stressors")
            .iter()
            .any(|stressor| stressor.as_str() == Some("ybr_full_pixels")),
        "YBR_FULL multi-frame RLE case should label YBR_FULL pixels as a stressor"
    );
    assert!(
    ...
    assert_eq!(
        rle_palette_file
            .pointer("/recipe/recipe_parameters/palette/descriptor")
            .and_then(Value::as_array)
            .map(Vec::len),
        Some(3)
    );
    ...
    assert!(
        rle_palette_file
            .pointer("/known_stressors")
            .and_then(Value::as_array)
            .expect("palette RLE manifest should include known stressors")
            .iter()
            .any(|stressor| stressor.as_str() == Some("palette_color_pixels")),
        "palette RLE case should label palette color pixels as a stressor"
    );
    assert!(
        validation_result_names(rle_palette_file.pointer("/validation/internal"))
            .contains(&"red_palette_lut_data"),
        "palette RLE manifest should retain Palette Color LUT validation"
    );
    assert!(
        validation_result_names(rle_palette_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
    ...
    let rle_mono1_multiframe_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono1_u8_multiframe_rle_lossless");
    assert_eq!(
        rle_mono1_multiframe_file
            .pointer("/image/photometric_interpretation")
            .and_then(Value::as_str),
        Some("MONOCHROME1")
    );
    ...
    assert!(
        validation_result_names(rle_mono1_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "MONOCHROME1 multi-frame RLE manifest should record decoded native frame hash validation"
    );
    let rle_u16_multiframe_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono2_u16_multiframe_rle_lossless");
    assert_eq!(
        rle_u16_multiframe_file
            .pointer("/image/frames")
            .and_then(Value::as_u64),
        Some(2)
    );
    ...
    assert!(
        validation_result_names(rle_u16_multiframe_file.pointer("/validation/internal"))
            .contains(&"number_of_frames"),
        "16-bit multi-frame RLE manifest should validate Number of Frames"
    );
    assert!(
        validation_result_names(rle_u16_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "16-bit multi-frame RLE manifest should record decoded native frame hash validation"
    );
    let rle_i16_multiframe_file =
        file_entry_by_case_id(&manifest, "classic/sc/mono2_i16_multiframe_rle_lossless");
    assert_eq!(
        rle_i16_multiframe_file
            .pointer("/image/pixel_representation")
            .and_then(Value::as_u64),
        Some(1)
    );
    ...
    assert!(
        validation_result_names(rle_mono1_u16_multiframe_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "16-bit MONOCHROME1 multi-frame RLE manifest should record decoded native frame hash validation"
    );
    let rle_odd_file =
    ...
    assert!(
        validation_result_names(cr_rle_file.pointer("/validation/internal"))
            .contains(&"cr_overlay_data"),
        "compressed CR manifest should retain Overlay Data validation"
    );
    assert!(
        validation_result_names(cr_rle_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "compressed CR manifest should record RLE decoded native frame hash validation"
    );
    let dx_rle_file = file_entry_by_case_id(
        &manifest,
    ...
    assert!(
        validation_result_names(dx_rle_file.pointer("/validation/internal"))
            .contains(&"dx_shutter_shape"),
        "compressed DX manifest should retain Display Shutter validation"
    );
    assert!(
        validation_result_names(dx_rle_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "compressed DX manifest should record RLE decoded native frame hash validation"
    );
    let mg_rle_file = file_entry_by_case_id(
        &manifest,
    ...
    assert!(
        validation_result_names(mg_rle_file.pointer("/validation/internal"))
            .contains(&"mg_view_code_sequence"),
        "compressed MG manifest should retain mammography View Code validation"
    );
    assert!(
        validation_result_names(mg_rle_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
    ...
    let us_rle_file = file_entry_by_case_id(&manifest, "classic/us/mono2_u8_rle_lossless");
    assert_eq!(
        us_rle_file
            .pointer("/dicom/sop_class_uid")
            .and_then(Value::as_str),
    ...
    assert!(
        validation_result_names(us_rle_file.pointer("/validation/internal"))
            .contains(&"us_ultrasound_color_data_present"),
        "compressed US manifest should retain Ultrasound Color Data Present validation"
    );
    assert!(
        validation_result_names(us_rle_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
    ...
    let mr_rle_file = file_entry_by_case_id(&manifest, "classic/mr/mono2_u16_rle_lossless");
    assert_eq!(
        mr_rle_file
            .pointer("/dicom/sop_class_uid")
            .and_then(Value::as_str),
    ...
    assert!(
        validation_result_names(mr_rle_file.pointer("/validation/internal"))
            .contains(&"mr_scanning_sequence"),
        "compressed MR manifest should retain MR Image validation"
    );
    assert!(
        validation_result_names(mr_rle_file.pointer("/validation/internal"))
            .contains(&"rle_decoded_frame_hashes"),
        "compressed MR manifest should record RLE decoded native frame hash validation"
    );
    if cfg!(feature = "jpeg") {
        let jpeg_file =
...
fn generate_command_writes_all_profile_union_and_skips_planned_cases() {
    ...
    file_entry_by_case_id(&manifest, "classic/sc/mono1_i16_rle_lossless");
