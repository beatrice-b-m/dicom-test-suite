...
pub fn oracle() -> Value {
    ...
    VERIFIED.call_once(|| {
        let output = Command::new("python3")
            ...
            .arg(Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/generic-cr-semantics.json"))
            .output().unwrap();
        assert!(output.status.success());
        assert_eq!(String::from_utf8(output.stdout).unwrap().trim(), "5ebae3fbf82ccacf52a1b782cdef53b5f046e8eff53d8a513ad64ac3349f1be6");
    });
    let value: Value =
        serde_json::from_slice(include_bytes!("../fixtures/generic-cr-semantics.json")).unwrap();
    assert_eq!(value["oracle_version"], "1.0.0");
    assert_eq!(
        value["source_receipt_sha256"],
        "1e8f489341d60babbdebd859a1bf3bd36c4937e9c0daeba70295e67fbd940350"
    );
    ...
    assert_eq!(row["caller_case"], "classic/mr/caller-radiography");
    assert_eq!(row["recipe_id"], "caller_radiography");
    assert_eq!(row["output_path"], "independent/radiography.dcm");
...
pub struct GenericCrBundle {
    pub root: PathBuf,
    pub members: PathBuf,
    pub descriptor: PathBuf,
    pub identity: Value,
}
impl GenericCrBundle {
    pub fn new() -> Self {
        ...
        let source =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/generic-cr-corpus");
        let expected = oracle();
        ...
        let registry: Value =
            serde_json::from_slice(&fs::read(members.join("cases/registry.json")).unwrap())
                .unwrap();
        // Preserve core membership while exercising a misleading MR case name.
        ...
        assert_eq!(
            identity,
            json!({"corpus_definition_sha256":"c101481618e81ad2bdd273d5821313f5c630e5b852081f8bb5a7ed162872aa48","definition_id":"fixture.generic-cr","definition_version":"1.0.0","file_count":3,"manifest_sha256":"7dcc1359ac74edbb40357e31453239c7517b5e1c33b4d9e85ff4b0bbd8219264","schema_version":"1.0.0","total_size_bytes":10055})
        );
        Self {
            root,
            members,
...
pub fn assert_semantics(file: &Value, row: &Value) {
    for key in ["image", "pixel_data", "expected_semantics"] {
        assert_eq!(file[key], row[key], "preserved CR field {key}");
    }
    assert_eq!(
...
pub fn assert_manifest(manifest: &Value, identity: &Value) {
    ...
    for (file, row) in files.iter().zip(rows) {
        ...
        assert_eq!(file["dicom"]["modality"], "CR");
        assert_eq!(file["dicom"]["sop_class_uid"], "1.2.840.10008.5.1.4.1.1.1");
