...
pub fn oracle() -> Value {
    ...
    VERIFIED.call_once(|| {
        let output = Command::new("python3")
            ...
            .arg(Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/generic-sc-semantics.json"))
            .output().unwrap();
        assert!(output.status.success());
        assert_eq!(String::from_utf8(output.stdout).unwrap().trim(), "f3cb9b1ba11c0d363a43254463883d74093e727caf94b03ed761af74912f597d");
    });
    let value: Value =
        serde_json::from_slice(include_bytes!("../fixtures/generic-sc-semantics.json")).unwrap();
    ...
    assert_eq!(value["cases"].as_array().unwrap().len(), variants.len());
    for (position, variant) in variants.into_iter().enumerate() {
        let row = &value["cases"][position];
        assert_eq!(
            row["source_case"],
            format!("classic/sc/{variant}_explicit_le")
        );
        assert_eq!(row["caller_case"], format!("caller/pixels/{variant}"));
        assert_eq!(row["recipe_id"], format!("caller_{variant}"));
        assert_eq!(row["output_path"], format!("independent/{variant}.dcm"));
        assert_eq!(row["profile"], if position < 3 { "smoke" } else { "core" });
    }
    value
}
...
pub struct GenericScBundle {
    pub root: PathBuf,
    pub members: PathBuf,
    pub descriptor: PathBuf,
    pub identity: Value,
}
impl GenericScBundle {
    pub fn new() -> Self {
        ...
        let source =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/generic-sc-corpus");
        let expected = oracle();
        ...
        let registry: Value =
            serde_json::from_slice(&fs::read(members.join("cases/registry.json")).unwrap())
                .unwrap();
        // This fixture intentionally places all thirteen independent caller cases in core.
        // Historical smoke/core membership is preserved in the separate original-case proofs.
        assert_eq!(registry["cases"].as_array().unwrap().len(), 13);
        ...
        assert_eq!(
            identity,
            json!({"schema_version":"1.0.0","definition_id":"fixture.generic-sc","definition_version":"1.0.0","file_count":15,"total_size_bytes":70145,"manifest_sha256":"5f2c0ecb9ea828598d84a079d5f11cbd154148d8fd7bfae3bf14f7e086b78f63","corpus_definition_sha256":"32726bd36c65de1aee646aa694c1451261e5137e7b88e14889ea026283009f28"})
        );
        Self {
            root,
            members,
...
pub fn assert_semantics(file: &Value, row: &Value) {
    for key in ["image", "pixel_data", "expected_semantics"] {
        assert_eq!(file[key], row[key], "preserved SC field {key}");
    }
    assert_eq!(
