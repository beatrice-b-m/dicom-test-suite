...
pub fn oracle() -> Value {
    ...
    VERIFIED.call_once(|| {
        let output = Command::new("python3")
            ...
            .arg(Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/generic-us-semantics.json"))
            .output().unwrap();
        assert!(output.status.success());
        assert_eq!(String::from_utf8(output.stdout).unwrap().trim(), "008a0426c4a395b4e47f34d45f49e5a75e2a160b00b4979fe56edc54c109e1b4");
    });
    let value: Value =
        serde_json::from_slice(include_bytes!("../fixtures/generic-us-semantics.json")).unwrap();
    ...
    assert_eq!(row["source_case"], "classic/us/mono2_u8_explicit_le");
    assert_eq!(row["caller_case"], "caller/acquisition/ultrasound");
    assert_eq!(row["recipe_id"], "caller_ultrasound");
    assert_eq!(row["output_path"], "independent/ultrasound.dcm");
...
pub struct GenericUsBundle {
    pub root: PathBuf,
    pub members: PathBuf,
    pub descriptor: PathBuf,
    pub identity: Value,
}
impl GenericUsBundle {
    pub fn new() -> Self {
        ...
        let source =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/generic-us-corpus");
        let expected = oracle();
        ...
        assert_eq!(
            identity,
            json!({"corpus_definition_sha256":"bd3f6f093032f97370fe164fe8a114c58b99299cd4edaf8e2544a7a444575004","definition_id":"fixture.generic-us","definition_version":"1.0.0","file_count":3,"manifest_sha256":"2fc7d9ace8a5e167def0d0701410350d39c7e81e4c9d5809ead622a1d22f7327","schema_version":"1.0.0","total_size_bytes":8174})
        );
        Self {
            root,
            members,
...
pub fn assert_semantics(file: &Value, row: &Value) {
    for key in ["image", "pixel_data", "expected_semantics"] {
        assert_eq!(file[key], row[key], "preserved US field {key}");
    }
    assert_eq!(
...
pub fn assert_manifest(manifest: &Value, identity: &Value) {
    ...
    for (file, row) in files.iter().zip(rows) {
        ...
        assert_eq!(file["dicom"]["modality"], "US");
        assert_eq!(
