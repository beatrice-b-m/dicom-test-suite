...
class ChangeTestRoutingFixtures(unittest.TestCase):
    ...
    def test_generic_pet_proof_routes_to_public_consumer_module(self):
        for path in ["tests/support/generic_pet_bundle.rs", "tests/fixtures/generic-pet-semantics.json", "tests/fixtures/generic-pet-corpus/definition.json"]:
            result = self.select(path)
            self.assertEqual(result["bundle_ids"], ["generic_pet_public_contract"])
            self.assertEqual(self.commands(result), ["cargo test --locked --no-default-features --test cli_sdk__nonfast external_corpus_cli::"])

    def test_generic_us_proof_routes_to_public_consumer_module(self):
        for path in ["tests/support/generic_us_bundle.rs", "tests/fixtures/generic-us-semantics.json", "tests/fixtures/generic-us-corpus/definition.json"]:
            result = self.select(path)
            self.assertEqual(result["bundle_ids"], ["generic_us_public_contract"])
            self.assertEqual(self.commands(result), ["cargo test --locked --no-default-features --test cli_sdk__nonfast external_corpus_cli::"])

    def test_generic_us_multiframe_proof_routes_to_public_consumer_module(self):
        for path in ["tests/support/generic_us_multiframe_bundle.rs", "tests/fixtures/generic-us-multiframe-semantics.json", "tests/fixtures/generic-us-multiframe-corpus/definition.json"]:
            result = self.select(path)
    ...
    def test_generic_ct_geometry_proof_routes_to_public_consumer_module(self):
        for path in ["tests/support/generic_ct_geometry_bundle.rs", "tests/fixtures/generic-ct-geometry-semantics.json", "tests/fixtures/generic-ct-geometry-corpus/definition.json"]:
            result = self.select(path)
    ...
    def test_caller_fixtures_reuse_external_corpus_contract_route(self):
        for fixture in ("generic-enhanced-corpus", "generic-sc-integer-corpus"):
    ...
    def test_corpus_definition_route_lists_all_thirty_eight_owned_tests(self):
        ...
        self.assertEqual(commands[0]["list_count"], 38)
