...
const INSTANCE_SHA256: &str = "9368e2b335876ae2c01b4ac5cf6c52e8c2875754e36b54efc928097c75da6dd6";
...
fn xa_monoplane_vertical_slice_is_exact_byte_stable_and_reported() {
    ...
    assert_eq!(
        dicom_test_suite::sha256_hex(&fs::read(first_root.join(RELATIVE_PATH)).unwrap()),
        INSTANCE_SHA256
    );
    assert_eq!(
    ...
    assert_eq!(summary.files_checked, 46);
...
fn validator_rejects_tampered_xa_projection_contract() {
    ...
    for (pointer, replacement, failure_key) in [
        (
            "/expected_xa_projection/image_type/2",
            json!("BIPLANE A"),
